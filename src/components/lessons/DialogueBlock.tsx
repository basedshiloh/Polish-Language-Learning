import { DialogueLine } from '@/lib/types';
import SpeakButton from '@/components/shared/SpeakButton';

interface DialogueBlockProps {
  lines: DialogueLine[];
}

export default function DialogueBlock({ lines }: DialogueBlockProps) {
  const speakers = [...new Set(lines.map((l) => l.speaker))];

  return (
    <div className="space-y-3">
      {lines.map((line, i) => {
        const isFirst = speakers.indexOf(line.speaker) === 0;
        return (
          <div
            key={i}
            className={`flex flex-col ${isFirst ? 'items-start' : 'items-end'}`}
          >
            <span className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1 px-1">
              {line.speaker}
            </span>
            <div
              className={`max-w-[85%] rounded-2xl px-4 py-3 ${
                isFirst
                  ? 'bg-blue-100 dark:bg-blue-900/40 rounded-tl-sm'
                  : 'bg-gray-100 dark:bg-gray-800 rounded-tr-sm'
              }`}
            >
              <div className="flex items-start gap-1">
                <p className="font-semibold text-blue-900 dark:text-blue-200 text-sm flex-1">{line.polish}</p>
                <SpeakButton text={line.polish} />
              </div>
              <p className="text-gray-500 dark:text-gray-400 text-xs mt-1">{line.english}</p>
            </div>
          </div>
        );
      })}
    </div>
  );
}
